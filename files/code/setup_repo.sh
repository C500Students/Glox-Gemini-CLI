...
# 1. Define target repository path
REPO_ROOT="$HOME/Glox-Gemini-CLI"
CODE_DIR="$REPO_ROOT/files/code"

echo "[*] Creating repository directory tree at $CODE_DIR..."
...
# 3. Write storage.rs (Redb metadata + Fjall LSM logs)
...
# 4. Write main.rs (Ratatui TUI + Tokio Gemini Async Dispatch)
cat << 'EOF' > src/main.rs
mod storage;
...
# 6. Initialize and push to remote
cd "$REPO_ROOT"
if [ ! -d ".git" ]; then
    git init
    git remote add origin https://github.com/C500Students/Glox-Gemini-CLI.git
fi

git add files/code/
git commit -m "feat: add complete Glox Gemini CLI TUI codebase with Redb and Fjall memory backends"
git branch -M main

echo "[+] Files generated and committed under files/code/."
echo "[+] Run 'git push -u origin main' to push changes upstream."
